#ifndef __LL_ALGO_H__
#define __LL_ALGO_H__
#include "llbase_algo.h"
#include "llfunctional.h"
#include "llheapalgo.h"
namespace LL {
...
ForwardIter1 search(ForwardIter1 first1, ForwardIter1 last1,
    ...
    auto d1 = LL::distance(first1, last1);
    auto d2 = LL::distance(first2, last2);
...
ForwardIter search_n(ForwardIter first, ForwardIter last, Size n,
    ...
    if (n <= 0) {
        ...
        first = LL::find(first, last, value);
        while (first != last) {
            auto m = n - 1;
            auto i = first;
            ++i;
...
ForwardIter1 find_end_dispatch(ForwardIter1 first1, ForwardIter1 last1,
    ...
    if (first2 == last2) {
        ...
        while (true) {
            ...
            auto new_result = LL::search(first1, last1, first2, last2);
            if (new_result == last1) {
                return result;
            } else {
                result = new_result;
...
BidirectionalIter1 find_end_dispatch(BidirectionalIter1 first1,
    ...
    reviter1 rresult =
        LL::search(reviter1(last1), rlast1, reviter2(last2), rlast2);
    if (rresult == rlast1) {
        return last1;
    } else {
        auto result = rresult.base();
        LL::advance(result, -LL::distance(first2, last2));
        return result;
    }
}
...
ForwardIter1 find_end(ForwardIter1 first1, ForwardIter1 last1,
    ...
    return LL::find_end_dispatch(first1, last1, first2, last2, Category1(),
                                    Category2());
}
...
ForwardIter1 find_end_dispatch(ForwardIter1 first1, ForwardIter1 last1,
    ...
    if (first2 == last2) {
        ...
        while (true) {
            // 利用 search 查找某个子序列的首次出现点，找不到则返回 last1
            auto new_result = LL::search(first1, last1, first2, last2, comp);
            if (new_result == last1) {
                return result;
            } else {
                result = new_result;
...
BidirectionalIter1 find_end_dispatch(
    BidirectionalIter1 first1, BidirectionalIter1 last1,
    ...
    reviter2 rlast2(first2);
    reviter1 rresult =
        LL::search(reviter1(last1), rlast1, reviter2(last2), rlast2, comp);
    if (rresult == rlast1) {
        return last1;
    } else {
        auto result = rresult.base();
...
ForwardIter lbound_dispatch(ForwardIter first, ForwardIter last, const T& value,
    ...
    auto len = LL::distance(first, last);
    auto half = len;
    ForwardIter middle;
    while (len > 0) {
        half = len >> 1;
        middle = first;
        LL::advance(middle, half);
        if (*middle < value) {
            first = middle;
            ++first;
            len = len - half - 1;
...
ForwardIter lower_bound(ForwardIter first, ForwardIter last, const T& value) {
    return LL::lbound_dispatch(first, last, value, iterator_category(first));
}
...
ForwardIter upper_bound(ForwardIter first, ForwardIter last, const T& value) {
    return LL::ubound_dispatch(first, last, value, iterator_category(first));
}
...
bool binary_search(ForwardIter first, ForwardIter last, const T& value) {
    auto i = LL::lower_bound(first, last, value);
...
template <class ForwardIter, class T>
LL::pair<ForwardIter, ForwardIter> erange_dispatch(ForwardIter first,
                                                      ForwardIter last,
                                                      const T& value,
                                                      forward_iterator_tag) {
    auto len = LL::distance(first, last);
    ...
    while (len > 0) {
        ...
        if (*middle < value) {
            ...
            left = LL::lower_bound(first, last, value);
            LL::advance(first, len);
            right = LL::upper_bound(++middle, first, value);
            return LL::pair<ForwardIter, ForwardIter>(left, right);
        }
    }
    return LL::pair<ForwardIter, ForwardIter>(last, last);
}

// erange_dispatch 的 random_access_iterator_tag 版本
template <class RandomIter, class T>
LL::pair<RandomIter, RandomIter> erange_dispatch(
    ...
    while (len > 0) {
        ...
        if (*middle < value) {
            ...
            left = LL::lower_bound(first, middle, value);
            right = LL::upper_bound(++middle, first + len, value);
            return LL::pair<RandomIter, RandomIter>(left, right);
        }
    }
    return LL::pair<RandomIter, RandomIter>(last, last);
}

template <class ForwardIter, class T>
LL::pair<ForwardIter, ForwardIter> equal_range(ForwardIter first,
                                                  ForwardIter last,
                                                  const T& value) {
    return LL::erange_dispatch(first, last, value, iterator_category(first));
}
...
LL::pair<ForwardIter, ForwardIter> erange_dispatch(ForwardIter first,
    ...
    while (len > 0) {
        ...
        if (comp(*middle, value)) {
            ...
            left = LL::lower_bound(first, last, value, comp);
            LL::advance(first, len);
            right = LL::upper_bound(++middle, first, value, comp);
            return LL::pair<ForwardIter, ForwardIter>(left, right);
        }
    }
    return LL::pair<ForwardIter, ForwardIter>(last, last);
...
template <class RandomIter, class T, class Compared>
LL::pair<RandomIter, RandomIter> erange_dispatch(RandomIter first,
                                                    RandomIter last,
                                                    const T& value,
                                                    random_access_iterator_tag,
                                                    Compared comp) {
    ...
    while (len > 0) {
        ...
        if (comp(*middle, value)) {
            ...
            left = LL::lower_bound(first, middle, value, comp);
            right = LL::upper_bound(++middle, first + len, value, comp);
            return LL::pair<RandomIter, RandomIter>(left, right);
        }
    }
    return LL::pair<RandomIter, RandomIter>(last, last);
...
ForwardIter remove(ForwardIter first, ForwardIter last, const T& value) {
    first = LL::find(first, last, value);  // 利用 find 找出第一个匹配的地方
    auto next = first;
    return first == last ? first
                         : LL::remove_copy(++next, last, first, value);
}
...
ForwardIter remove_if(ForwardIter first, ForwardIter last,
                      UnaryPredicate unary_pred) {
    first = LL::find_if(first, last,
                           unary_pred);  // 利用 find_if 找出第一个匹配的地方
    auto next = first;
    return first == last
               ? first
               : LL::remove_copy_if(++next, last, first, unary_pred);
}
...
void reverse_dispatch(RandomIter first, RandomIter last,
                      random_access_iterator_tag) {
    while (first < last) LL::iter_swap(first++, --last);
}

template <class BidirectionalIter>
void reverse(BidirectionalIter first, BidirectionalIter last) {
    LL::reverse_dispatch(first, last, iterator_category(first));
}
...
void random_shuffle(RandomIter first, RandomIter last) {
    ...
    for (auto i = first + 1; i != last; ++i) {
        LL::iter_swap(i, first + (rand() % (i - first + 1)));
    }
}
...
void random_shuffle(RandomIter first, RandomIter last,
    ...
    auto len = LL::distance(first, last);
    for (auto i = first + 1; i != last; ++i) {
        LL::iter_swap(i, first + (rand(i - first + 1) % len));
    }
}
...
BidirectionalIter rotate_dispatch(BidirectionalIter first,
                                  ...
                                  bidirectional_iterator_tag) {
    LL::reverse_dispatch(first, middle, bidirectional_iterator_tag());
    LL::reverse_dispatch(middle, last, bidirectional_iterator_tag());
    while (first != middle && middle != last) LL::swap(*first++, *--last);
    if (first == middle) {
        LL::reverse_dispatch(middle, last, bidirectional_iterator_tag());
        return last;
    } else {
...
RandomIter rotate_dispatch(RandomIter first, RandomIter middle, RandomIter last,
    ...
    if (l == r) {
        LL::swap_ranges(first, middle, last);
        return result;
    }
...
ForwardIter rotate(ForwardIter first, ForwardIter middle, ForwardIter last) {
    ...
    return LL::rotate_dispatch(first, middle, last,
...
ForwardIter rotate_copy(ForwardIter first, ForwardIter middle, ForwardIter last,
                        OutputIter result) {
    return LL::copy(first, middle, LL::copy(middle, last, result));
}
...
bool is_permutation_aux(ForwardIter1 first1, ForwardIter1 last1,
    ...
    constexpr bool is_ra_it =
        LL::is_random_access_iterator<ForwardIter1>::value &&
        LL::is_random_access_iterator<ForwardIter2>::value;
    if (is_ra_it) {
    ...
    if (is_ra_it) {
        ...
        auto len1 = LL::distance(first1, last1);
        auto len2 = LL::distance(first2, last2);
...
bool is_permutation(ForwardIter1 first1, ForwardIter1 last1,
    ...
    static_assert(std::is_same<v1, v2>::value,
                  "the type should be same in LL::is_permutation");
...
bool next_permutation(BidirectionalIter first, BidirectionalIter last) {
    ...
    for (;;) {
        ...
        if (*--i < *ii) {  // 找到第一对小于关系的元素
            ...
            LL::iter_swap(i, j);    // 交换 i，j 所指元素
            LL::reverse(ii, last);  // 将 ii 之后的所有元素反转
            return true;
        }
        if (i == first) {
            LL::reverse(first, last);
            return false;
        }
    }
}
...
OutputIter merge(InputIter1 first1, InputIter1 last1, InputIter2 first2,
    ...
    return LL::copy(first2, last2, LL::copy(first1, last1, result));
}
...
void merge_without_buffer(BidirectionalIter first, BidirectionalIter middle,
    ...
    if (len1 + len2 == 2) {
        if (*middle < *first) LL::iter_swap(first, middle);
        return;
    }
    ...
    if (len1 > len2) {  // 序列一较长，找到序列一的中点
        len11 = len1 >> 1;
        LL::advance(first_cut, len11);
        second_cut = LL::lower_bound(middle, last, *first_cut);
        len22 = LL::distance(middle, second_cut);
    } else {  // 序列二较长，找到序列二的中点
        len22 = len2 >> 1;
        LL::advance(second_cut, len22);
        first_cut = LL::upper_bound(first, middle, *second_cut);
        len11 = LL::distance(first, first_cut);
    }
    auto new_middle = LL::rotate(first_cut, middle, second_cut);
    LL::merge_without_buffer(first, first_cut, new_middle, len11, len22);
    LL::merge_without_buffer(new_middle, second_cut, last, len1 - len11,
                                len2 - len22);
}
...
BidirectionalIter1 merge_backward(BidirectionalIter1 first1,
    ...
    if (first1 == last1) return LL::copy_backward(first2, last2, result);
    if (first2 == last2) return LL::copy_backward(first1, last1, result);
    --last1;
    --last2;
    while (true) {
        if (*last2 < *last1) {
            *--result = *last1;
            if (first1 == last1)
                return LL::copy_backward(first2, ++last2, result);
            --last1;
        } else {
            *--result = *last2;
            if (first2 == last2)
                return LL::copy_backward(first1, ++last1, result);
            --last2;
        }
    }
}
...
BidirectionalIter1 rotate_adaptive(BidirectionalIter1 first,
    ...
    if (len1 > len2 && len2 <= buffer_size) {
        buffer_end = LL::copy(middle, last, buffer);
        LL::copy_backward(first, middle, last);
        return LL::copy(buffer, buffer_end, first);
    } else if (len1 <= buffer_size) {
        buffer_end = LL::copy(first, middle, buffer);
        LL::copy(middle, last, first);
        return LL::copy_backward(buffer, buffer_end, last);
    } else {
        return LL::rotate(first, middle, last);
    }
}
...
void merge_adaptive(BidirectionalIter first, BidirectionalIter middle,
    ...
    if (len1 <= len2 && len1 <= buffer_size) {
        Pointer buffer_end = LL::copy(first, middle, buffer);
        LL::merge(buffer, buffer_end, middle, last, first);
    } else if (len2 <= buffer_size) {
        Pointer buffer_end = LL::copy(middle, last, buffer);
        LL::merge_backward(first, middle, buffer, buffer_end, last);
    } else {  // 区间长度太长，分割递归处理
        auto first_cut = first;
        auto second_cut = middle;
        Distance len11 = 0;
        ...
        auto new_middle =
            LL::rotate_adaptive(first_cut, middle, second_cut, len1 - len11,
                                   len22, buffer, buffer_size);
        LL::merge_adaptive(first, first_cut, new_middle, len11, len22,
                              buffer, buffer_size);
        LL::merge_adaptive(new_middle, second_cut, last, len1 - len11,
...
void inplace_merge_aux(BidirectionalIter first, BidirectionalIter middle,
    ...
    auto len1 = LL::distance(first, middle);
    auto len2 = LL::distance(middle, last);
    temporary_buffer<BidirectionalIter, T> buf(first, last);
    if (!buf.begin()) {
        LL::merge_without_buffer(first, middle, last, len1, len2);
    } else {
        LL::merge_adaptive(first, middle, last, len1, len2, buf.begin(),
                              buf.size());
    }
}
...
void inplace_merge(BidirectionalIter first, BidirectionalIter middle,
    ...
    LL::inplace_merge_aux(first, middle, last, value_type(first));
}
...
void merge_without_buffer(BidirectionalIter first, BidirectionalIter middle,
    ...
    if (len1 == 0 || len2 == 0) return;
    if (len1 + len2 == 2) {
        if (comp(*middle, *first)) LL::iter_swap(first, middle);
        return;
    }
    auto first_cut = first;
    auto second_cut = middle;
    ...
    if (len1 > len2) {
        len11 = len1 >> 1;
        LL::advance(first_cut, len11);
        second_cut = LL::lower_bound(middle, last, *first_cut, comp);
        len22 = LL::distance(middle, second_cut);
    } else {
        len22 = len2 >> 1;
        LL::advance(second_cut, len22);
        first_cut = LL::upper_bound(first, middle, *second_cut, comp);
        len11 = LL::distance(first, first_cut);
    }
    auto new_middle = LL::rotate(first_cut, middle, second_cut);
    LL::merge_without_buffer(first, first_cut, new_middle, len11, len22,
...
void merge_adaptive(BidirectionalIter first, BidirectionalIter middle,
    ...
    if (len1 <= len2 && len1 <= buffer_size) {
        Pointer buffer_end = LL::copy(first, middle, buffer);
        LL::merge(buffer, buffer_end, middle, last, first, comp);
    } else if (len2 <= buffer_size) {
        Pointer buffer_end = LL::copy(middle, last, buffer);
        LL::merge_backward(first, middle, buffer, buffer_end, last, comp);
    } else {  // 区间长度太长，分割递归处理
        auto first_cut = first;
        auto second_cut = middle;
        Distance len11 = 0;
...
void inplace_merge_aux(BidirectionalIter first, BidirectionalIter middle,
    ...
    temporary_buffer<BidirectionalIter, T> buf(first, last);
    if (!buf.begin()) {
        LL::merge_without_buffer(first, middle, last, len1, len2, comp);
    } else {
        LL::merge_adaptive(first, middle, last, len1, len2, buf.begin(),
                              buf.size(), comp);
    }
...
void inplace_merge(BidirectionalIter first, BidirectionalIter middle,
    ...
    if (first == middle || middle == last) return;
    LL::inplace_merge_aux(first, middle, last, value_type(first), comp);
}
...
void partial_sort(RandomIter first, RandomIter middle, RandomIter last) {
    LL::make_heap(first, middle);
    for (auto i = middle; i < last; ++i) {
        if (*i < *first) {
            LL::pop_heap_aux(first, middle, i, *i, distance_type(first));
        }
    }
    LL::sort_heap(first, middle);
}
...
void partial_sort(RandomIter first, RandomIter middle, RandomIter last,
                  Compared comp) {
    LL::make_heap(first, middle, comp);
    for (auto i = middle; i < last; ++i) {
        if (comp(*i, *first)) {
            LL::pop_heap_aux(first, middle, i, *i, distance_type(first),
                                comp);
    ...
    LL::sort_heap(first, middle, comp);
}
...
RandomIter psort_copy_aux(InputIter first, InputIter last,
    ...
    LL::make_heap(result_first, result_iter);
    while (first != last) {
        if (*first < *result_first) {
            LL::adjust_heap(result_first, static_cast<Distance>(0),
                               result_iter - result_first, *first);
        }
        ++first;
    }
    LL::sort_heap(result_first, result_iter);
    return result_iter;
}
...
RandomIter partial_sort_copy(InputIter first, InputIter last,
                             RandomIter result_first, RandomIter result_last) {
    return LL::psort_copy_aux(first, last, result_first, result_last,
...
RandomIter psort_copy_aux(InputIter first, InputIter last,
    ...
    LL::make_heap(result_first, result_iter, comp);
    while (first != last) {
    ...
    LL::sort_heap(result_first, result_iter, comp);
    return result_iter;
}
...
// 的输出区间 其余放到 result_false 的输出区间，并返回一个 LL::pair
// 指向这两个区间的尾部
/*****************************************************************************************/
template <class InputIter, class OutputIter1, class OutputIter2,
          class UnaryPredicate>
LL::pair<OutputIter1, OutputIter2> partition_copy(
    ...
    return LL::pair<OutputIter1, OutputIter2>(result_true, result_false);
}
...
void intro_sort(RandomIter first, RandomIter last, Size depth_limit) {
    while (static_cast<size_t>(last - first) > kSmallSectionSize) {
        if (depth_limit == 0) {  // 到达最大分割深度限制
            LL::partial_sort(first, last, last);  // 改用 heap_sort
            return;
        }
        --depth_limit;
        auto mid =
            LL::median(*(first), *(first + (last - first) / 2), *(last - 1));
        auto cut = LL::unchecked_partition(first, last, mid);
        LL::intro_sort(cut, last, depth_limit);
        last = cut;
    }
}
...
void unchecked_insertion_sort(RandomIter first, RandomIter last) {
    for (auto i = first; i != last; ++i) {
        LL::unchecked_linear_insert(i, *i);
    }
}
...
void insertion_sort(RandomIter first, RandomIter last) {
    ...
    for (auto i = first + 1; i != last; ++i) {
        auto value = *i;
        if (value < *first) {
            LL::copy_backward(first, i, i + 1);
            *first = value;
        } else {
            LL::unchecked_linear_insert(i, value);
        }
    }
}
...
void final_insertion_sort(RandomIter first, RandomIter last) {
    if (static_cast<size_t>(last - first) > kSmallSectionSize) {
        LL::insertion_sort(first, first + kSmallSectionSize);
        LL::unchecked_insertion_sort(first + kSmallSectionSize, last);
    } else {
        LL::insertion_sort(first, last);
    }
}
...
void sort(RandomIter first, RandomIter last) {
    if (first != last) {
        ...
        LL::intro_sort(first, last, slg2(last - first) * 2);
        LL::final_insertion_sort(first, last);
    }
}
...
void intro_sort(RandomIter first, RandomIter last, Size depth_limit,
    ...
    while (static_cast<size_t>(last - first) > kSmallSectionSize) {
        if (depth_limit == 0) {  // 到达最大分割深度限制
            LL::partial_sort(first, last, last, comp);  // 改用 heap_sort
            return;
        }
        --depth_limit;
        auto mid =
            LL::median(*(first), *(first + (last - first) / 2), *(last - 1));
        auto cut = LL::unchecked_partition(first, last, mid, comp);
        LL::intro_sort(cut, last, depth_limit, comp);
        last = cut;
    }
}
...
void unchecked_insertion_sort(RandomIter first, RandomIter last,
                              Compared comp) {
    for (auto i = first; i != last; ++i) {
        LL::unchecked_linear_insert(i, *i, comp);
    }
}
    ...
    for (auto i = first + 1; i != last; ++i) {
        ...
        if (comp(value, *first)) {
            ...
            *first = value;
            ...
            LL::unchecked_linear_insert(i, value, comp);
        }
    }
}
...
void final_insertion_sort(RandomIter first, RandomIter last, Compared comp) {
    if (static_cast<size_t>(last - first) > kSmallSectionSize) {
        LL::insertion_sort(first, first + kSmallSectionSize, comp);
        LL::unchecked_insertion_sort(first + kSmallSectionSize, last, comp);
    } else {
        LL::insertion_sort(first, last, comp);
    }
}
...
void sort(RandomIter first, RandomIter last, Compared comp) {
    if (first != last) {
        // 内省式排序，将区间分为一个个小区间，然后对整体进行插入排序
        LL::intro_sort(first, last, slg2(last - first) * 2, comp);
        LL::final_insertion_sort(first, last, comp);
    }
}
...
void nth_element(RandomIter first, RandomIter nth, RandomIter last) {
    if (nth == last) return;
    while (last - first > 3) {
        auto cut = LL::unchecked_partition(
            first, last,
            LL::median(*first, *(first + (last - first) / 2), *(last - 1)));
...
OutputIter unique_copy(InputIter first, InputIter last, OutputIter result) {
    ...
    return LL::unique_copy_dispatch(first, last, result,
...
ForwardIter unique(ForwardIter first, ForwardIter last) {
    first = LL::adjacent_find(first, last);
    return LL::unique_copy(first, last, first);
}

// 重载版本使用函数对象 comp 代替比较操作
template <class ForwardIter, class Compared>
ForwardIter unique(ForwardIter first, ForwardIter last, Compared comp) {
    first = LL::adjacent_find(first, last, comp);
    return LL::unique_copy(first, last, first, comp);
}
