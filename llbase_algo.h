#ifndef __LL_BASE_ALGO_H__
#define __LL_BASE_ALGO_H__
#include <cstring>
#include <type_traits>
...
OutputIter unchecked_copy_cat(InputIter first, InputIter last,
                              OutputIter result, LL::input_iterator_tag) {
...
LL::pair<InputIter, OutputIter> unchecked_copy_n(InputIter first, Size n,
                                                 OutputIter result,
                                                 LL::input_iterator_tag) {
    for (; n > 0; --n, ++first, ++result) {
        *result = *first;
    }
    return LL::pair<InputIter, OutputIter>(first, result);
}

template <class RandomIter, class Size, class OutputIter>
LL::pair<RandomIter, OutputIter> unchecked_copy_n(
    RandomIter first, Size n, OutputIter result,
    LL::random_access_iterator_tag) {
    auto last = first + n;
    return LL::pair<RandomIter, OutputIter>(last,
                                            LL::copy(first, last, result));
}

template <class InputIter, class Size, class OutputIter>
LL::pair<InputIter, OutputIter> copy_n(InputIter first, Size n,
                                       OutputIter result) {
...
BidirectionalIter2 unchecked_move_backward_cat(BidirectionalIter1 first,
                                               ...
                                               BidirectionalIter2 result,
                                               LL::bidirectional_iterator_tag) {
    while (first != last) *--result = LL::move(*--last);
    return result;
}
...
RandomIter2 unchecked_move_backward_cat(RandomIter1 first, RandomIter1 last,
                                        RandomIter2 result,
                                        LL::random_access_iterator_tag) {
    for (auto n = last - first; n > 0; --n) *--result = LL::move(*--last);
    return result;
}
...
bool lexicographical_compare(const unsigned char* first1,
    ...
    // 先比较相同长度的部分
    const auto result = std::memcmp(first1, first2, LL::min(len1, len2));
    // 若相等，长度较长的比较大
...
/*****************************************************************************************/
template <class InputIter1, class InputIter2>
LL::pair<InputIter1, InputIter2> mismatch(InputIter1 first1, InputIter1 last1,
                                          InputIter2 first2) {
    while (first1 != last1 && *first1 == *first2) {
        ++first1;
        ++first2;
    }
    return LL::pair<InputIter1, InputIter2>(first1, first2);
}
